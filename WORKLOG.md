...
### 2026-07-10 — ECS Service Auto Scaling (P4 심화, 브랜치 `feat/ecs-autoscaling-p4`, ADR-0012)
...
- 결정 & 이유(why) — 상세 근거는 [ADR-0012](./docs/adr/0012-ecs-service-autoscaling.md)에 전부 기록, 요지만:
...
- 산출물: `infra/terraform/autoscaling.tf`(신규)·`variables.tf`(`autoscaling_enabled`·`autoscaling_max`)·`ecs.tf`(주석 1줄)·`outputs.tf`(`autoscaling_status`). 문서 `docs/adr/0012-ecs-service-autoscaling.md`(신규)·`docs/adr/README.md`(테이블 추가).
...
- 관련: 브랜치 `feat/ecs-autoscaling-p4`, ADR-0012, CLAUDE.md §7(오토스케일링 P4 additive 지시)·§0.2(과설계 금지)·로드맵 P4, ADR-0011(위험 성격 비교 기준점), D1(k6 부하테스트, 짝 산출물).
