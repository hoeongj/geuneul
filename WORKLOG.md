...
> **아카이브:** 시간순 개발 기록이다. 각 항목의 "다음 할 일", 단계명, 당시 인프라 수치는 기록 시점의 판단이며 현재 작업 목록이 아니다. 현재 기능·배포 상태는 [README](./README.md)와 [DEPLOY](./DEPLOY.md)를 기준으로 한다.

> **목적:** 주요 기술 결정의 배경(**무엇을 / 왜(why) / 검토한 대안 / 트렌드 근거**)을 시간순으로 남기는 개발 일지.
> 언제 무슨 작업을 했는지 + **스펙·방법·로직·알고리즘·라이브러리를 왜 그렇게 골랐는지(why)** 를 남긴다.

## 작성 규칙
- 마감 전에는 **작업할 때마다 맨 아래에 append**했다. 재개 시에도 기존 기록을 덮어쓰지 않고 시간순으로 추가한다.
...
- 한 일: GitHub public 레포 생성(hoeongj/geuneul) + push → **CI 실제 GREEN**(GitHub 러너에서 PostGIS/Redis 통합테스트·Flyway·GiST까지 통과, 1m33s). 이후 배포(CD) 방식을 재검토.
- 결정 & 이유(why):
...
- 관련: `docs/SPEC.md`(§7 인프라·§10 P5 개정), 레포 `github.com/hoeongj/geuneul`, CI run 28539003609(success)
...
## 2026-07-11 — F3: 화장실 경로 도로 폴리라인 (카카오내비, 기존 키 재사용, ADR-0021)
- 한 일(백엔드): `KakaoDirectionsProvider`(`DirectionsProvider` 구현, `@Primary`) — `POST apis-navi.kakaomobility.com/v1/waypoints/directions` 호출 → `routes[].sections[].roads[].vertexes`(경도,위도) → LatLng 폴리라인. 키 없음/2점 미만/실패/`result_code!=0`이면 empty → RouteService 직선 폴백(graceful). 단위테스트 5건(파싱·result_code≠0·HTTP오류·무키 무호출·2점 미만). 한 일(프론트): `RouteMiniMapLive`/`RouteMiniMap`(react-kakao-maps-sdk `Polyline`+bounds fit, road=실선 파랑/straight=점선 회색+배지), `PlaceDetailOverlay`에 경로 상태+상단 미니맵 폴리라인 오버레이+지도 초기화.
...
- 왜(why): ① **왜 zerodep인가(핵심)** — ADR-0018이 F2를 미룬 사유가 `nl.martijndwars:web-push`의 **BouncyCastle fat-jar 서명 프로바이더 리스크**(로컬 미검출·Docker에서만 터지는 TS-009류, 이 맥은 colima 막혀 Docker 검증 불가). `zerodep-web-push-java`는 **JDK 내장 crypto만** 써 그 리스크를 원천 제거 → 방어적 선택. JDK11+·Boot4 호환. ② **왜 플래그 게이팅인가** — `push.enabled=false`(기본)면 VAPID 빈 없음→`sendToUser` no-op→인앱 센터(B1) 회귀 0. 검증 안 된 크립토를 blind ship 안 함(§0-2·TS-026). 켜는 순간 살아나게 배선만 완료. 구독 저장은 플래그 무관(미리 받아두고 나중에 켜기 가능). ③ **왜 채널 분리인가** — Web Push는 인앱 delivery와 병렬. 발송 지점(HEAT_ESCAPE)만 재사용해 additive 호출, 실패 격리. 급증(벌크 INSERT) push는 대상 회수 복잡·중복 위험이라 follow-up. ④ **왜 /push/test인가** — 실기기 배너 수신은 iOS 설치형 PWA에서만 검증 가능 → 한 탭 end-to-end 확인 진입점.
- 검증: 라이브러리 키 계약검증(TS-026 — 생성 VAPID PEM을 zerodep로 로드→87자 base64url 공개키 확인). 단위 `PushServiceTest` 6건(활성/비활성 분기·구독 upsert·비활성 no-op)·`NotificationServiceTest` 9건 green. 프론트 tsc·eslint green. CI 게이트(NotificationFlowIT 컨텍스트 — push.enabled=false, VAPID 빈 없음, 정상 기동).
...
## 2026-07-11 — 세션 마무리: 결정 3건 + 버그 진단 2건 + 다음 세션 계획(N1~N9)·`/geuneul-finish`
...
- **N2 댓글 "개수만 보임"**: **가설(읽기경로 없음) 기각.** 읽기 경로 완비·`CommunityFlowIT`가 댓글 텍스트 왕복 검증(green). 카운트·내용이 같은 fetch 배열이라 펼친 상태에선 못 갈라짐. 진짜 원인 = **접힘(collapse) UX**(카운트는 접힌 헤더, 내용은 `{open && ...}`) 또는 **stale 배포**. 해법: 프론트 UX(작성 후 자동 펼침/카운트 노출 조건) + 배포 최신화. **백엔드 무변경.**
...
- **무엇**: 상단 검색바가 "준비 중" 토스트만 띄우던 스텁을 **실제 카카오 로컬 키워드 검색**으로 구현. 검색 → 결과 목록 → 선택 시 지도가 그 좌표로 이동(+검색 핀) → onIdle bounds 조회가 그 주변 우리 마커를 채운다.
  - 백엔드: `domain.search` 신규 — `KakaoKeywordClient`(`/v2/local/search/keyword.json`, `KakaoAK` 헤더, record 역직렬화)·`KeywordSearchController`(`GET /places/search?query=&lat=&lng=&size=`, permitAll)·`PlaceSearchResult` DTO. 키는 백엔드에만(BFF), 클라 노출 0.
...
- **무엇**: 마이페이지 "내 활동" — 내가 쓴 **후기/댓글/유용해요**를 탭으로 모아보고 각 항목에서 원문 장소 상세로 이동.
  - 백엔드 `domain.activity` 신규: `MyActivityController`(`GET /me/reviews`·`/me/comments`·`/me/reactions`, 인증)·`MyActivityService`·프로젝션 3종(`MyReviewView`/`MyCommentView`/`MyReactionView`)·DTO 3종. 각 리포지토리에 유저필터 조인 쿼리 추가(`ReviewRepository.findByUserIdWithPlace`·`ReviewCommentRepository.findByUserIdWithPlace`·`ReactionRepository.findMyReviewReactions`). SecurityConfig에 3경로 authenticated 추가.
...
- **무엇**: 후기 닉네임 탭 → **작성자 공개 프로필**(닉네임·신뢰도·팔로워 수·공개 후기) → **팔로우** → 마이페이지 **팔로잉**(나만)에서 재방문. 팔로워 "목록"은 어디에도 안 뜬다(카운트만).
  - 백엔드 `domain.follow` 신규: V17 `follows`(UNIQUE 멱등 + CHECK 자기팔로우 금지) · `Follow`·`FollowRepository`·`FollowService`·`UserController`(`GET /users/{id}` 공개·`POST/DELETE /users/{id}/follow` 인증·`GET /me/following` 인증) · DTO 3종(UserProfile/Follow/Following). SecurityConfig 3경로. **후기 목록은 N6 `findByUserIdWithPlace` 재사용**. `ReviewResponse`에 `authorId` 추가(닉네임→프로필).
  - 프론트: `selectedUser` 컨텍스트 + `UserProfileOverlay`(장소 상세와 동일 오버레이 패턴, z-50) + `FollowingSection`(마이페이지) + `ReviewsSection` 닉네임 버튼화 + `lib/api`·`queries`·BFF `/api/users/[id]`·`/api/users/[id]/follow`·`/api/me/following` + `proxyOptionalAuth`(공개지만 로그인 시 following 채우는 GET).
...
- **무엇(백엔드)**: `PlaceRepository.findBestToiletWaypoint`(고정 `category='TOILET'`)를 `findBestWaypointByCategories`(`category = ANY(string_to_array(:categories,','))`, `p.category` 프로젝션)로 일반화·대체 · `RouteWaypointView.getCategory()` 추가 · `RouteService.toiletRoute`를 공통 `viaRoute(from,to,categoriesCsv)`로 추출→`toiletRoute`=TOILET·`shadeRoute`=SHADE_CATEGORIES(쉼터·도서관·지하상가) · `RouteController` GET /routes/shade(미러·공개·requireKorea) · `RouteResponse.RouteStop.category`(경유지만) 노출.
...
- **무엇**: `frontend/public/geuneul.apk`(서명된 TWA, 2.5MB, package `app.vercel.geuneul.twa`) + `frontend/public/.well-known/assetlinks.json`(Digital Asset Links, 서명 SHA-256 = `E6:14:B7:4B:86:0E:C9:7A:48:02:24:FB:FC:5B:ED:D3:7F:41:D2:F8:85:77:6E:20:DE:D3:4B:C9:F0:12:D9:E3`) + `InstallView`에 "APK 파일 직접 다운로드"(안드로이드, iOS는 숨김) + 사이드로딩 안내.
- **왜(why) + 2026 웹검증(규칙B)**: ① **Bubblewrap 1.24.1**(GoogleChromeLabs, 현행)로 web manifest→서명 APK 생성 — PWA→네이티브 TWA 래핑. ② **assetlinks SHA-256이 keystore 서명과 일치해야** TWA가 주소창 없이 전체화면으로 뜸(불일치면 반쪽) → apksigner 검증으로 APK 서명 cert(`e614b74b...`)=keystore fingerprint 일치 확인. ③ **자체배포는 APK만**(AAB는 Play 업로드 전용, 사이드로딩 불가) → `.apk`를 public에 호스팅(Vercel 정적 서빙). ④ **WebAPK를 여전히 메인**(경고 0), APK는 "파일로 받고 싶을 때" 보조 — install 안내에 명시.
- **빌드 방법(재현)**: JDK 17(mise 17.0.2) + Android SDK(cmdline-tools 13114758·build-tools;34.0.0·platforms;android-34, sdkmanager 무인 라이선스 수락) → `~/.bubblewrap/config.json`에 jdk/sdk 경로 시드(대화형 위저드 회피) → keytool로 keystore 생성(`.local/android-twa.keystore`, 비번 `.local/android-twa.env`, **둘 다 gitignore·§D**) → `twa-manifest.json`(package·host·아이콘·색·splashScreenFadeOutDuration=300) → `bubblewrap update`(프로젝트 생성) → `bubblewrap build --skipPwaValidation`(BUBBLEWRAP_KEYSTORE_PASSWORD/KEY_PASSWORD env 무인 서명).
...
## 2026-07-14 — 최종 UX·문서 마감

프로젝트 종료 전 실사용 피드백과 공개 문서를 다시 대조해, 동작 변경은 필요한 버그 수정에만 한정하고 문서는 현재 구현과 과거 기록을 분리했다.

- **현재 위치 재중심 수정:** 지도 드래그 후 현재 위치 버튼이 성공 토스트만 보이고 실제로 이동하지 않는 문제를 확인했다. 위치 API가 아니라 지도 SDK가 동일한 중심 props를 변경으로 인식하지 않는 것이 원인이었다. `KakaoMapLive`가 지도 인스턴스의 `panTo`를 직접 호출하도록 변경해 드래그 상태와 무관하게 재중심한다(TS-036).
- **문서 정합성:** `frontend/README.md`를 초기 MVP 설명에서 현재 클라이언트 구조·기능·환경 변수·검증·배포 설명으로 교체했다. `design-brief`·`SPEC`·`WORKLOG`는 당시 설계/기록임을 명시하고 현재 상태의 단일 출처를 README·architecture로 연결했다. 후속 구현으로 전제가 바뀐 ADR(알림·경로·RDS 보안)은 당시 결정을 보존하면서 상단에 후속 상태를 추가했다.
- **검증:** Markdown 상대 링크 41개 resolve, `pnpm typecheck`, `pnpm lint`, `pnpm build`, `./gradlew test jacocoTestCoverageVerification` 성공. 최신 `main`에서 Vercel 배포와 GitHub Actions(Frontend·Backend·gitleaks)도 성공 확인.
- **마감 판단:** 기능 코드와 문서의 현재 상태를 분리해 유지보수자가 과거 로드맵을 현재 미완료 항목으로 오해하지 않도록 했고, 새 기능·의존성·인프라 변경은 추가하지 않았다. 이후 변경은 별도 요구가 생길 때 새 이력으로 시작한다.
- **관련:** TS-036, ADR-0029, 커밋 `71c2a46`·`2537164`.
