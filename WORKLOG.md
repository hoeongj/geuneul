...
- 검증: 실 API 프로브로 계약 4종 실측 확정(래퍼 없음·숫자 좌표·I21201/R10202·서버필터 동작). 단위 14건 green(`SmallBusinessStoreApiClientTest` 실응답 픽스처 재작성 4 · `StoreCategoryMapperTest` 코드분류 5 · `StoreIngestionServiceTest` 코드별 분리·격자집계 5). 전체 `./gradlew test` green. 프로덕션 실적재·실측은 머지→배포 후(아래 후속 append 예정).
