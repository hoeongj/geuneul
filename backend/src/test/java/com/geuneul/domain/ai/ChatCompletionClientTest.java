...
/**
 * 프로바이더 중립 OpenAI 호환 Chat Completions 요청/응답 계약을 실 직렬화기로 검증한다(TS-004 교훈 — 모킹은
...
class ChatCompletionClientTest {
    ...
    private static final String BASE_URL = "https://chat-completions.test/api/v1";
    ...
    void extractsAndTrimsContent() {
        ChatCompletionClient client = new ChatCompletionClient("test-key", "qwen/qwen3-next-80b-a3b-instruct:free", BASE_URL, builder);
    ...
    void emptyWhenNoChoices() {
        ChatCompletionClient client = new ChatCompletionClient("test-key", "any-model", BASE_URL, builder);
        server.expect(requestTo(BASE_URL + "/chat/completions"))
                .andRespond(withSuccess("""
                        {"choices":[]}
                        """, MediaType.APPLICATION_JSON));
    ...
    void emptyWhenNoKey() {
        ChatCompletionClient noKey = new ChatCompletionClient("", "any-model", BASE_URL, RestClient.builder());
    ...
    void emptyWhenBlankKey() {
        ChatCompletionClient blankKey = new ChatCompletionClient("   ", "any-model", BASE_URL, RestClient.builder());
