...
/**
 * 추천 결과 한 건 (ADR-0008). 장소(+지도와 동일한 canonical survival 배지)에
 * 시나리오 적합도(matchScore)와 근거(reason)를 얹는다.
 *
 * <p><b>두 점수의 의미가 다르다(의도):</b> {@code place.survival}은 §5 표준 가중치로 낸
 * "이 장소의 지금 상태"(지도 마커와 동일 배지)이고, {@code matchScore}는 시나리오 가중치로 낸
 * "이 상황에 얼마나 맞나"(이 목록의 정렬 기준)다.
 */
@Schema(description = "추천 결과 — 장소 + 시나리오 적합도 + 근거")
public record RecommendationResponse(
        @Schema(description = "장소(지도와 동일한 survival 배지 포함)") PlaceResponse place,
