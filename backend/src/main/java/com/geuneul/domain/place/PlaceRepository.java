...
public interface PlaceRepository extends JpaRepository<Place, Long> {
    ...
    /** 반경 검색: 중심(lat,lng)에서 radiusMeters 이내, 가까운 순. */
    @Query(value = """
    ...
    /** kNN 최근접: 반경 제한 없이 가까운 순 상위 N개 (<-> 연산자, index-assisted KNN). */
    @Query(value = """
            SELECT p.* FROM places p
            ...
            LIMIT :limit
            """, nativeQuery = true)
    List<Place> findNearest(@Param("lat") double lat,
                            @Param("lng") double lng,
                            @Param("category") String category,
                            @Param("limit") int limit);

    /** bounds(뷰포트) 검색: west,south,east,north 박스 안의 마커. */
    @Query(value = """
            SELECT p.* FROM places p
