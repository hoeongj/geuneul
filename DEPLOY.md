...
> 원칙: mp가 자가호스팅 k3s를 증명 → 그늘은 **회사들이 실제 쓰는 AWS**로 그 갭을 보완. EKS($73/월)는 배제(mp가 k8s 이미 증명). 비용은 $200 크레딧 + RDS 프리티어 + ECS 무료 control plane + NAT 없음으로 최소화(상시 비용은 ALB ~$16/월 + Fargate ~$12/월).
>
...
## 비용 메모
- 상시: **ALB ~$16/월 + Fargate 태스크(0.25 vCPU/1GB 상시 1개) ~$12/월**(Fargate는 프리티어 없음). RDS(db.t3.micro)는 프리티어, ECS control plane 무료, SSM/ECR 사실상 무료. → $200 크레딧으로 수개월 커버.
...
# REST 키는 셸 환경변수로만 전달(스크립트가 태스크에 주입 — 레포 하드코딩 금지, 규칙 D):
