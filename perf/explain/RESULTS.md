...
## 2. V8 튜닝 — place_report_signals 뷰의 유효 제보 필터 (before/after)
...
```
docker compose up -d
cd backend && ./gradlew bootRun            # Flyway V1~V8 적용
# 대량 시드(별도 셸)
docker exec -i <postgres> psql -U geuneul -d geuneul -v n=300000 -f - < perf/seed/seed_synthetic_places.sql
# 실행계획
