...
<details>
<summary><b>구현 이력 (W0 → P3)</b></summary>
...
- **P2** — 휘발성 제보(reports) 풀스택 라이브(익명, 11개 타입·타입별 TTL, `SEAT_OK`/`CROWDED` 포함). 레이트리밋 XFF 신뢰경계·OOM 하드닝(적대적 리뷰, TS-008).
- **P3** — `survival_score`(SQL 뷰 + 순수 함수, 마커 3색·상태 배지, [ADR-0007](./docs/adr/0007-survival-score-sql-signals-java-compose.md)) + 시나리오 추천 `/recommendations`([ADR-0008](./docs/adr/0008-recommendations-scenario-weighted-ranking.md)).
