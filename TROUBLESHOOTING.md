...
- **면접 어필 포인트:** ① **검증 회복력** — 테스트 인프라가 죽어도 "무엇을 확신해야 하나(뷰 SQL 시맨틱)"로 되돌아가 실 PostGIS에 마이그레이션+시나리오를 직접 돌려 확증. ② 컨테이너 런타임 내부(docker-java API 버전 협상, XFF와 무관한 소켓/DOCKER_HOST 상속, Gradle forked JVM env)까지 원인을 좁히는 디버깅. ③ skip을 "통과"로 오독하지 않는 규율 — 스킵된 테스트 수를 결과 XML로 명시 확인하고 CI로 실제 실행을 보증.
