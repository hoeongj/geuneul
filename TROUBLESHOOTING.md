...
## TS-032 — BFF 프록시가 204(No Content)를 빈 문자열 바디로 재구성 → Response 생성자 TypeError → 502 오탐
**상황**: F2 실기기에서 "테스트" 누르면 프론트는 "테스트 발송에 실패했어요"인데 **OS 배너는 실제로 뜬다**(푸시는
성공). TS-029(동기→비동기)로 이미 한 번 고쳤는데도 프론트가 여전히 실패로 본다.

**원인**: 백엔드 `/push/test`는 `@ResponseStatus(NO_CONTENT)` → **204**를 준다(발송은 async fire-and-forget이라
바로 204). BFF 프록시(`lib/backend.ts` proxyAuthed 등)는 응답을 `const body = await res.text()`(204는 "")로 읽어
`new NextResponse(body, { status: 204 })`로 재구성한다. 그런데 Fetch 표준상 **204/205/304(null-body status)에
non-null 바디(빈 문자열 "" 포함)를 주면 Response 생성자가 `TypeError: Invalid response status code 204`를 던진다**
(`node -e 'new Response("",{status:204})'`로 재현). 이 throw가 프록시 try/catch에 걸려 **502로 오변환** →
프론트 `res.ok=false` → "실패". 즉 **백엔드는 성공(배너 발송), 프록시가 응답 재구성에서 자멸**. push subscribe/test,
알림 read/규칙삭제 등 **모든 204 프록시 경로**가 동일 영향(조용히 실패로 보였음).

**해결**: 프록시가 빈 바디를 **null**로 넘긴다 — `new NextResponse(resBody || null, { status })`. null-body status에
null 바디는 합법이라 throw 없음. 4개 프록시 헬퍼(proxyPost/proxyAuthed/proxyPhotoPresign/proxyOptionalAuth) 일괄 적용.
