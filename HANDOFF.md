...
> - **라이브 현황**: 태스크데프 **rev60**(rev59=F2 이미지 자동배포 + rev60=push 활성 수동 rev: `PUSH_ENABLED=true`·VAPID public/subject env·private key SSM secret). Flyway **V16(push_subscriptions)**까지. API·App 200. **`/push/public-key` enabled=true·87자 검증.** `/routes/toilet` mode=road. 데이터 = 쉼터 60,297(냉방 57,070)·화장실 52,334·도서관 3,551·**상권 서울+6대 광역시+9개 도시**. ADR 최신 **0022**. VAPID 키 `.local/vapid*.pem`+SSM `/geuneul/push_vapid_private_key_pem`(§D).
> - **남은 것 = `docs/BACKLOG.md` F1(중소도시 잔여)·F2(실기기 확인)·F4·F6**: **F1** 나머지 중소도시(쿼터만, 멱등·순차) · **F2** 활성화 후 실기기(설치형 PWA) `/push/test` OS 배너 1회 확인 · **F4** 그늘/비 경로(경로 주변 쉼터 오버레이, F3 완료로 착수가능) · **F6** A9 Fargate cpu(트래픽 대기). ~~F3·F5~~ ✅완료.
> - **다음 세션**: `/geuneul-start`가 `docs/BACKLOG.md`를 구동. **코드 백로그 F1~F5 소진**(F6만 트래픽 대기) — F4(그늘 경로 설계)·F1 잔여 도시가 다음 후보.
