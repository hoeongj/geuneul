...
> - **라이브 현황**: 태스크데프 **rev56**(F5는 마이그레이션·secret 무변경, `backend/**` push 자동배포로 롤아웃 완료·health 200/notifications 401 검증). Flyway **V15**까지 적용. API·App 200. 데이터 = 쉼터 60,297(냉방 57,070) · 화장실 52,334 · 도서관 3,551 · **상권 서울 distinct 29,886 + 6대 광역시 + 9개 도시**. ADR 최신 **0020**.
> - **남은 것 = `docs/BACKLOG.md` F1(중소도시 잔여)·F2~F4·F6**(외부 대기·후속): **F1** 나머지 중소도시(쿼터만, 멱등·순차) · **F2** B1 Web Push 전송(VAPID 있음, 실기기 검증 필요) · **F3** B2 카카오모빌리티 도로 폴리라인(활용신청 키) · **F4** 그늘/비 경로(F3 의존) · ~~**F5** HEAT_ESCAPE~~(✅완료) · **F6** A9 Fargate cpu(트래픽 대기).
> - **다음 세션**: `/geuneul-start`가 `docs/BACKLOG.md`를 구동. **코드 백로그는 F5까지 소진** — F1 잔여 도시(쿼터)만 즉시 가능, 나머지는 사용자 키/실기기/트래픽 준비 후.
