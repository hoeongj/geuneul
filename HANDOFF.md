...
- **미착수/후속 — 남은 건 전부 "사용자 콘솔 승인 대기" 블로커 2개**: ① **쉼터 전국(safetydata)** — 전국무더위쉼터 표준데이터 API가 `resultCode 12`(미승인/폐기), safetydata 전용 키도 없음. 풀려면 data.go.kr에서 무더위쉼터 표준데이터 **활용신청 승인**받거나 포털에서 전국 파일 수동 다운로드(로그인 필요). ② **상권정보 STUDY_CAFE/CAFE** — 같은 datago 키로 library=정상(resultCode 00)인데 상가업소 API(B553077)만 **403 Forbidden** = **활용신청 미승인**. data.go.kr에서 그 API 활용신청 승인 후 가능. (~~EventBridge 스케줄~~ #42 ENABLED · ~~ALB HTTPS~~ #43 CloudFront · ~~화장실 재적재~~ +5,437 · ~~AI 유효키~~ #41 Mistral — 전부 완료.)
...
- **인프라:** AWS ECS Fargate(+**오토스케일링** min1/max3 CPU60%) + RDS PostgreSQL(PostGIS) + **ElastiCache Redis** + **S3**(사진) + **EventBridge Scheduler**(주기동기화, **ENABLED** 월1회) + **CloudFront**(무료 HTTPS 기본 도메인, ADR-0015) + Terraform(IaC) + GitHub Actions OIDC + ECR + ALB. `main`에 `backend/**` push 시 자동배포. **현재 라이브 태스크데프 rev41**(REDIS_HOST·S3_BUCKET_NAME·AWS_REGION·**AI_SUMMARY_BASE_URL·AI_SUMMARY_MODEL** env + SSM secret **10종**=DB·PROXY·KMA·KAKAO_REST·KAKAO_SECRET·GOOGLE_ID·GOOGLE_SECRET·JWT·**AI_SUMMARY·DATA_GO_KR**). 라이브 태스크데프는 `ignore_changes`라 secret/env 추가 시 **수동 rev 등록**(describe→추가→register→update-service). `infra/`.
- **데이터(프로덕션 RDS):** 무더위쉼터 100건(전국 샘플 — 전국 재적재는 API 활용신청 승인 대기) + 공중화장실 **52,334건**(카카오 지오코딩 — 2026-07-10 실패 7,193 재시도로 46,897→+5,437, 잔여 1,756은 카카오가 못 푸는 옛 지번·산번지) + 도서관(전국 3,551건, library API 멱등). 광화문·대전·부산·강릉·창원 라이브 검증 통과.
