...
> 🎉 **2026-07-11 — `docs/BACKLOG.md` C1~C4(2차·심화 마무리) 전량 완료·머지·라이브(PR #96~#99). `/geuneul-finish` 일괄 구동.**
> 각 항목 기능 브랜치 → PR → **`gh pr checks` Backend/Next.js pass 눈확인(TS-025)** → squash 머지 → 배포. 커밋 신원 `hoengj`, Co-Authored-By 없음. **C1·C3·C4는 적대적 리뷰 워크플로(다렌즈→검증)로 확정 결함을 커밋 전 선제 수정.**
> - **C2 a11y(#96, 프론트)**: `lib/hooks.ts` `useDialogFocusTrap`(Esc+포커스+Tab트랩 DRY, nested aria-modal 양보)·`useIsLg`(useSyncExternalStore) 신설. 두 오버레이 통합(트랩 `!(onMap&&isLg)`). SearchBar `useId` 콤보박스(role combobox/listbox/option·화살표 nav).
...
> 각 기능 브랜치 → PR → **`gh pr checks` Backend pass 눈확인(TS-025)** → squash 머지 → 프로덕션 배포·라이브 검증. 커밋 신원 `hoengj`, Co-Authored-By 없음.
...
> - **데스크톱 반응형(2026-07-11, PR #92)**: 실사용 피드백("PC에선 폰 앱처럼만 뜸") → 데스크톱(≥lg=1024px) **지도앱 표준 3분할**(풀블리드 지도 + 좌측 사이드바 + 좌측 내비 레일). 신규 `NavRail`·`MapSidebar`·`PlaceListBody`, 지도/오버레이/콘텐츠 `lg:` 분기, **모바일 무변경**. 전수 감사(5렌즈·21건)로 **오버레이 라우트 스코핑 회귀 버그**(오버레이 `lg:w-[400px]` 좌측패널이 지도 아닌 탭에 새어나감 → `usePathname()==='/'` 가드)·hover/cursor/focus/Esc·문서 정합성까지 #92에 함께 머지. 백엔드 orphan `@Component` 제거·경로 javadoc(#93)·오버레이 `role=dialog`+포커스 in/복귀(#94) 별도 머지·라이브.
> - **다음 = `docs/BACKLOG.md`의 C1~C4**(2차·심화 마무리 — 2026-07-11 스펙 워크플로로 **현황·수정위치(파일)·수용기준까지 박음**, 재조사 불필요): **C1** 신고/모더레이션 프론트 진입점(백엔드 완비·마이그레이션 0·§9 최소·비노출) · **C2** a11y 심화(오버레이 Tab 트랩 `!(onMap&&isLg)` + 검색 콤보박스 `useId`, 프론트 전용) · **C3** 관심 장소 상태변화 알림(북마크 장소 단건 침수/미끄럼·**V9 이벤트 재사용**·기존 BOOKMARK_SURGE 규칙 병합·ADR) · **C4** 그늘 경유 경로(F3 대칭 경유지 1곳·**자체 라우팅 금지 §0-2**·ADR). **`/geuneul-finish`가 C1~C4를 한 번에 구동**(`/geuneul-start`는 현황 검증). 외부 블로커 0건.
...
- **직전 상태(2026-07-10 P4-백로그 세션)**: 외부 승인 불필요 백로그 ①~⑨ 전부 완료·배포·프로덕션 실측(PR #44~#53, 핫픽스 #52). Flyway **V9~V12** 프로덕션 무사 적용(급증 트리거·verified·커뮤니티·모더레이션 hidden). 새 라이브 엔드포인트 실측 200: `/alerts/surge`·`/alerts/stream`(SSE)·`/places/{id}/popular-times`·`/recommendations?scenario=focus|longstay`·상세 `features`/`aiSummary`·`/admin/flags?status=`·`/reviews/{id}/comments`·`/reactions`. JaCoCo floor 0.60→**0.70**.
  ...
  - **커밋 규칙 신설(CLAUDE.md §A)**: `Co-Authored-By: Claude` 트레일러 금지(GitHub contributors에 hoengj만). 과거 39개 커밋은 이 세션에 history rewrite로 제거·force-push 완료(트리 불변, author 전부 유저).
