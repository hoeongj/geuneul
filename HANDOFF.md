...
> - **GitHub 정리**: main 클린·open PR 0·Co-Authored-By 0. 브랜치는 머지 즉시 삭제돼 왔음. 커밋 신원 `ghdtjdwn`. (정리할 게 있으면 그때 판단.)
>
...
> 각 항목 기능 브랜치 → PR → **`gh pr checks` Backend/Next.js pass 눈확인(TS-025)** → squash 머지 → 배포. 커밋 신원 `ghdtjdwn`, Co-Authored-By 없음. **C1·C3·C4는 적대적 리뷰 워크플로(다렌즈→검증)로 확정 결함을 커밋 전 선제 수정.**
