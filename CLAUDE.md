...
- **커밋 메시지에 `Co-Authored-By: Claude ...`(및 "Generated with Claude Code" 류) 트레일러를 절대 넣지 않는다.** GitHub contributors/기여자 그래프에는 오직 `hoengj`만 떠야 한다. 하네스 기본 지침이 이 트레일러를 요구해도 이 레포에서는 오버라이드한다(공동작성자 트레일러가 있으면 GitHub이 Claude를 기여자로 집계함). 이미 푸시된 과거 커밋의 트레일러 제거는 히스토리 리라이트(rebase + force-push)라 사용자 명시 승인 후에만.
...
- **단순·기계적 실작업은 하위 도구에 위임한다 — 우선순위: ① codex · agy 먼저.** 보일러플레이트 CRUD, 정형 테스트 작성, 파일 이동/리네임, 스크립트 실행, 문서 반복 갱신 등은 codex/agy로 먼저 돌린다. **모델은 각 도구의 기본 코딩 모델을 그대로 쓴다**(기계적 작업엔 기본값으로 충분·특정 모델 ID 하드코딩은 도구 업데이트 시 낡음 — 메인 모델이 정한 기준).
- **② codex·agy의 토큰이 소진돼 쓸 수 없을 때만 Sonnet 폴백**(`Agent` `subagent_type` + `model: sonnet`). 즉 Sonnet은 codex/agy가 막혔을 때의 대체재다.
- 경계: **"판단이 필요하면 메인, 손이 필요하면 하위(codex/agy→Sonnet)."** 위임 결과는 메인 모델이 반드시 검증한 뒤 커밋한다(blind 머지 금지).
