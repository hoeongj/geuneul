...
> 이 문서는 프로젝트의 목표·범위·데이터·알고리즘·ERD·API 계약을 정의하는 단일 스펙이다. 코드·ADR·트러블슈팅에서 "§n"으로 참조한다. 개별 의사결정 근거는 [`adr/`](./adr), 작업 이력은 [`../WORKLOG.md`](../WORKLOG.md).
...
| **P3 · 스코어·추천·AI** | survival_score + 요약 | 시공간 랭킹(SQL), 추천 시나리오, 날씨 API+Redis TTL 캐시, AI 한줄 요약, **공공데이터 주기 동기화**(EventBridge→ECS RunTask, 멱등 upsert 재실행 + 스냅샷 이탈 행 soft-delete) |
| **P4 · 심화** | 성능·실시간·관측 | **k6 부하테스트 + EXPLAIN 인덱스 튜닝**, 실시간 이벤트(제보 급증 알림), 캐시 전략, 관측성(OTel/Grafana), **ADR 문서** |
