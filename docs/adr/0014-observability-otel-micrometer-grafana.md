...
- 선행: CLAUDE.md §7(Test/Ops: OpenTelemetry/Grafana)·§10 P4(관측성)·ADR-0012(k6 부하테스트 — 같은
...
CLAUDE.md §7 원문이 "micrometer-tracing-bridge-otel + OTLP exporter, 또는 Boot의 OTel 스타터" 둘 다
...
CLAUDE.md의 핵심 차별점("PostGIS 대용량 지리검색(반경/kNN)")을 그대로 계측 대상으로 삼았다 — bounds는
...
- **Tempo(single-binary 모드)**: 별도 OTel Collector 없이 백엔드가 OTLP(gRPC 4317 / HTTP 4318)를
  Tempo에 **직접 push**한다 — Collector를 하나 더 얹는 건 이 규모(포트폴리오, 트래픽 적음)에
...
- 로컬에서 `docker compose --profile observability up -d`만으로 Prometheus·Grafana·Tempo가 뜨고,
  실제 latency(p95/p99)·트레이스를 볼 수 있다 — 포트폴리오 시연 가능한 상태(대시보드·트레이스 검색
  스크린샷을 즉시 뜰 수 있음).
...
- CLAUDE.md §0-2(과설계 금지)·§7(Test/Ops: OpenTelemetry/Grafana)·§10 P4(관측성)·
