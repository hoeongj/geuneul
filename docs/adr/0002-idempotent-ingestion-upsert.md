...
- 상태: 승인 (2026-07-02)
- 관련: `places (source, source_external_id)` UNIQUE, `PlaceBulkUpsertRepository`, `CoolingShelterCsvParser`

## 문제(Context)

공공데이터(무더위쉼터, 공중화장실 52k+)는 **주기적으로 갱신**되고, 인제스천은 실패·재실행이
...
3. **파일 단위 단일 트랜잭션** — 절반만 반영된 상태를 없앤다. 실패 시 전체 롤백 후 재실행.
4. **좌표 결측·범위 밖 행은 버리지 않고 계수(skipped)** — 지오코딩 보완(P2, 카카오 로컬 API)
   대상 규모를 수치로 남긴다.
...
| 대안 | 기각 이유 |
|---|---|
| JPA `saveAll()` + 사전 존재 조회 | 조회+더티체킹으로 N배 느리고, 조회-삽입 사이 경합 여지. 52k행 스케일에 부적합 |
