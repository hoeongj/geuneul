...
- 관련: ADR-0019(화장실 경로 MVP·`DirectionsProvider` 전략), `domain.route`, `KakaoGeocodingClient`(같은 키),
  프론트 `RouteMiniMap`/`PlaceDetailOverlay`, SPEC.md §0-4(공간쿼리)·§7(카카오)·TS-026(계약검증)
- 후속: F3 완료. 그늘·비 경로는 ADR-0024, 그늘 경유 경로는 ADR-0027에서 완료.
...
- **비용 주석**: 카카오모빌리티는 2026년 다중경유지에 건당 과금(할인가 ~10원)을 도입했으나, 현재
...
- **자체 가중 라우팅(그늘 경로)**: §0-2 범위 밖으로 유지했고, 후속에서는 ADR-0024의 corridor 오버레이와 ADR-0027의 경유지 방식으로 단순화했다.

## 결과(Consequences)

- (+) 사용자 액션·새 비밀 0으로 도로 폴리라인 라이브. 기존 키·간판 공간쿼리(경유지 선택) 재사용.
- (+) 키 없는 환경은 자동 직선 폴백 — 기존 계약·테스트·CI 무변경.
- (−) 다중경유지 과금 가능성 — 현재 규모엔 무시 가능, 필요 시 2-leg 무료 전환점 확보.
