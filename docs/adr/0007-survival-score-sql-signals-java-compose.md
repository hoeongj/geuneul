...
| open_now를 중립값 0.5로 채움 | 없는 데이터를 지어내는 것이라 방어 불가. 재정규화로 "모르면 빼기"가 정직 |
...
- 공식·제약: SPEC.md §5(survival_score)·§6(공포 조장 금지)·§8(ERD reports/reviews 분리)
