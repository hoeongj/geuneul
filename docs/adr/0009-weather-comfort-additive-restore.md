...
SPEC.md §5 comfort_score 정의("시원함/앉을 곳/콘센트/물/화장실")에 원래 "시원함"이 들어있었으므로,
...
(추측값 대신 공식 기준선 — SPEC.md §0-B "방어 가능한 선택" 원칙):
...
- 다음 자연스러운 확장: `rain` 추천 시나리오(ADR-0008)의 risk 성분에도 **실시간** 강수(PTY)를 반영하는 것 —
  지금은 UGC FLOOD/SLIPPERY 제보만 risk에 반영되고 날씨 API의 실시간 강수는 아직 안 쓰인다(범위 확대 전 별도 제안 필요, SPEC.md §0-2).
...
- 공식·제약: SPEC.md §4(날씨·Redis TTL 캐시 필수)·§5(survival_score comfort_score)·§7(AI/데이터 소스),
